...
LOG_FILE="$LOG_DIR/contacts_$(date +%Y%m%d_%H%M%S).log"

echo "[$(date "+%Y-%m-%d %H:%M:%S")] 开始备份通讯录" | tee -a "$LOG_FILE"
echo "[$(date "+%Y-%m-%d %H:%M:%S")] 输出目录: $OUTPUT_DIR" | tee -a "$LOG_FILE"
echo "----------------------------------------" | tee -a "$LOG_FILE"

for retry in 1 2 3; do
    am start -n org.fossify.contacts/.activities.HeadlessBackupActivity \
